...
impl DiskIO {
    fn stats(device_name: &str) -> Result<Self, Box<dyn Error>> {
    ...
    fn refresh(&mut self, device_name: &str) -> Result<(), Box<dyn Error>> {
...
impl DiskSpace {
    fn stats() -> Result<Option<Self>, Box<dyn Error>> {
...
impl DiskInfo {
    pub fn new() -> Self {
        match DiskInfo::root_device_name() {
            Ok(Some(device_name)) => {
            ...
            Ok(None) => {
                eprintln!("Can not find the root device name");
                Self {
                    device_name: None,
                    space: None,
                    io: None,
    ...
    pub fn root_device_name() -> Result<Option<String>, Box<dyn Error>> {
        let buffer = fs::read_to_string("/proc/mounts")?;
        for line in buffer.lines() {
